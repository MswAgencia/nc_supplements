...
class ControllerSaleOrder extends Controller {
	...
	public function info() {
		...
		if ($order_info) {
			...
			$data['token'] = $this->session->data['postoken'];
			...
			$data['invoice'] = $this->url->link('sale/order/invoice', 'token=' . $this->session->data['postoken'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');
			$data['cancel'] = $this->url->link('common/home', 'token=' . $this->session->data['postoken'] . $url, 'SSL');
			...
			if ($order_info['customer_id']) {
				$data['customer'] = $this->url->link('sale/customer/update', 'token=' . $this->session->data['postoken'] . '&customer_id=' . $order_info['customer_id'], 'SSL');
			} else {
				$data['customer'] = '';
			}
			...
			foreach ($products as $product) {
				...
				$data['products'][] = array(
					...
					'href'     		   => $this->url->link('catalog/product/update', 'token=' . $this->session->data['postoken'] . '&product_id=' . $product['product_id'], 'SSL')
				);
			}
			...
			foreach ($vouchers as $voucher) {
				$data['vouchers'][] = array(
					...
					'href'        => $this->url->link('sale/voucher/update', 'token=' . $this->session->data['postoken'] . '&voucher_id=' . $voucher['voucher_id'], 'SSL')
				);
			}
	...
	public function history() {
		...
		$pagination->limit = 10; 
		$pagination->text = $this->language->get('text_pagination');
		$pagination->url = $this->url->link('sale/order/history', 'token=' . $this->session->data['postoken'] . '&order_id=' . $this->request->get['order_id'] . '&page={page}', 'SSL');
