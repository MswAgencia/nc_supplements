...
class ControllerCommonHome extends Controller {   
	...
	public function chart() {
		...
		switch ($range) {
			case 'day':
				for ($i = 0; $i < 24; $i++) {
					$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE payment_code='".'cash_'.$this->user->getId()."' AND order_status_id = '" . (int)$pos['config_pos_complete_status_id'] . "' AND (DATE(date_added) = DATE(NOW()) AND HOUR(date_added) = '" . (int)$i . "') GROUP BY HOUR(date_added) ORDER BY date_added ASC");
					if ($query->num_rows) {
			...
			case 'week':
				...
				for ($i = 0; $i < 7; $i++) {
					...
					$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE payment_code='".'cash_'.$this->user->getId()."' AND order_status_id = '" . (int)$pos['config_pos_complete_status_id'] . "' AND DATE(date_added) = '" . $this->db->escape($date) . "' GROUP BY DATE(date_added)");
			...
			case 'month':
				for ($i = 1; $i <= date('t'); $i++) {
					...
					$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE payment_code='".'cash_'.$this->user->getId()."' AND order_status_id = '" . (int)$pos['config_pos_complete_status_id'] . "' AND (DATE(date_added) = '" . $this->db->escape($date) . "') GROUP BY DAY(date_added)");
			...
			case 'year':
				for ($i = 1; $i <= 12; $i++) {
					$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE payment_code='".'cash_'.$this->user->getId()."' AND order_status_id = '" . (int)$pos['config_pos_complete_status_id'] . "' AND YEAR(date_added) = '" . date('Y') . "' AND MONTH(date_added) = '" . $i . "' GROUP BY MONTH(date_added)");
	...
	public function login() {
		...
		if (isset($this->request->get['token'])) {
			$login_data = $this->cache->get('pos.'.trim($this->request->get['token']));
			if($login_data)
			{
				$this->session->data['postoken'] = trim($this->request->get['token']);
				$this->session->data['user_id'] = $login_data['user_id'];
				$this->session->data['store_id'] = $login_data['store_id'];
			}
		}
